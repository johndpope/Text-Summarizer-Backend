...
def cosineSimilarity(title, sentence):
	...
	firstSum = sum([title[word]**2 for word in title.keys()])
	secondSum = sum([sentence[word]**2 for word in sentence.keys()])
...
def inverseSentFreq(tokenizedText):
	...
	for token in allTokens:
		containToken = map(lambda sent: token in sent, tokenizedText)
